...
@section('title')
    @lang('transAdmin.admin-panel')
@endsection
@section('content')
    <div class="d-flex align-items-center justify-content-between pb-4">
        <div class="h3 mb-3">@lang('transAdmin.admin-panel')</div>
        <div class="d-inline-flex">
            <div class="dropdown mr-2">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dmb1">
                    <a class="dropdown-item" href="{!! route('admin.admin-panel', ['api' => 1, 'robot' => $robot]) !!}">
                        @lang('transAdmin.api')
                    </a>
                    <a class="dropdown-item" href="{!! route('admin.admin-panel', ['api' => 0, 'robot' => $robot]) !!}">
                        @lang('transAdmin.web')
                    </a>
                </div>
            </div>
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dmb2">
                    <a class="dropdown-item" href="{!! route('admin.admin-panel', ['api' => $api, 'robot' => 0]) !!}">
                        @lang('transAdmin.human')
                    </a>
                    <a class="dropdown-item" href="{!! route('admin.admin-panel', ['api' => $api, 'robot' => 1]) !!}">
                                            ...
                                            <tr class="{{ $visitor->suspect_hits > 20 ? 'text-danger' : '' }}">
                                                <td>
                                                    @if ($visitor->ipAddress->country_code)
                                                        <img src="{{ asset('flags/' . $visitor->ipAddress->country_code . '.png') }}"
                                                            alt="{{ $visitor->ipAddress->country_name }}"
                                                            class="rounded-0 mr-1 h-auto w-auto">
                                                    @else
                                                        <img src="{{ asset('flags/flag.png') }}"
                                                            alt="{{ $visitor->ipAddress->ip_address }}"
                                                            class="rounded-0 h-auto w-auto mr-1">
                                                    @endif
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="4" class="text-center bg-light text-secondary">
                                                    <i class="fas fa-exclamation-circle"></i> @lang('transAdmin.visitor')
                                            ...
                                            <tr>
                                                <td>
                                                    @if ($country->country_code)
                                                        <img src="{{ asset('flags/' . $country->country_code . '.png') }}"
                                            ...
                                            <tr>
                                                <td>
                                                    @if ($suspectVisitor->ipAddress->country_code)
                                                        <img src="{{ asset('flags/' . $suspectVisitor->ipAddress->country_code . '.png') }}"
                                            ...
                                            <tr>
                                                <td>
                                                    @if ($suspectCountry->country_code)
                                                        <img src="{{ asset('flags/' . $suspectCountry->country_code . '.png') }}"
                                                            alt="{{ $suspectCountry->country_code }}"
                                                            class="rounded-0 h-auto w-auto mr-1">
                                                    @else
                                                        <img src="{{ asset('flags/flag.png') }}"
