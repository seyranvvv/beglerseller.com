...
@section('content')
    <div class="justify-content-between align-items-center flex-wrap">
        <div class="float-left">
            <h4 class="mb-3 mb-md-0 ">Eşikler</h4>
        </div>

        <div class="d-flex align-items-center flex-wrap text-nowrap float-right">
            <a href="{{ route('dresses.create') }}" class="btn btn-primary btn-icon-text mb-2 mb-md-0"><i data-feather="plus"
                                ...
                                <tbody>
                                    @foreach ($dresses as $dress)
                                        <tr>
                                            ...
                                            <td>{{ $dress->name }} </td>
                                            <td> {{ price_format($dress->price) }} </td>
                                            <td>{{ $dress->order }}</td>
                                            <td>{{ $dress->active ? 'Hawa' : 'Ýok' }}</td>
                                            <td>{{ $dress->views }}</td>
                                            <td><img src="{{ $dress->getFirstMediaUrl('dresses') }}" alt="dress">
                                            </td>
                                            <td>
                                                <div class="chat-footer d-flex">
                                                    <a href="{{ route('dresses.edit', $dress) }}"
                                                    ...
                                                    <form action="{{ route('dresses.destroy', $dress) }}" method="post"
                                                        class="delete-dress">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
...
@push('js')
    <script>
        $(document).on('submit', '.delete-dress', function(e) {
